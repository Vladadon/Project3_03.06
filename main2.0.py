...
async def read_departments(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
async def create_departments(department: DepartmentsCreate,db: Session = Depends(get_db)):
    db_departments = department(department_name=department.department_name, manager_name=department.manager_name, phone_number=department.phone_number, daily_sales_volume=department.daily_sales_volume)
    try:
